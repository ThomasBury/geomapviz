...
import matplotlib
import holoviews
from geomapviz.geomapviz import (
    prepare_geo_data,
    ...
    bel_df = pd.DataFrame(
        {
            ...
            "truth": feat_1,
            "feat_2": feat_1 + np.random.beta(0.5, 0.5, size=len(feat_1)),
...
class TestPrepareGeoData:
    def test_prepare_geo_data(self):
        ...
        message = "The output df is not the right shape, the shape is {}, expected (589, 19)".format(
            geo_df.shape
        )
...
class TestAggregator:
    def test_single_predictor_aggr_multi_mod(self):
        ...
        message = "The output df is not the right shape, the shape is {}, expected (5890, 6)".format(
            df_average.shape
        )
        assert df_average.shape == (5890, 6), message

        message = "The output df is not the right shape, the shape is {}, expected (5890, 5)".format(
            df_.shape
        )
...
class TestPlotOnMap:
    def test_plot_on_map(self):
        ...
        message = (
            "not the right type, got {} but expected matplotlib.figure.Figure".format(
                str(type(f))
            )
        )
